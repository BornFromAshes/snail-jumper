...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [3, 3, 1]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        # as inputs we give the player nearest obstacle on left side and right side
        # and nearest obstacle in the middle(if any). and the number of obstacles on each side
        # TODO normalize
        inputs = np.zeros((3, 1))
        min_y_left = -100
        min_y_right = -100
        ...
        for obs in obstacles:
            if obs['x'] == 177:
                ...
                if obs['y'] > min_y_right:
                    min_y_right = obs['y']
            else:
                if obs['y'] > min_y_mid:
                    min_y_mid = obs['y']

        inputs[0][0] = max((min_y_left - 456) / 200, 0)
        inputs[1][0] = max((min_y_mid - 456) / 200, 0)
        inputs[2][0] = max((min_y_right - 456) / 200, 0)
